    ...
    for video in videos:
        ...
        while 1:
            ...
            for c in cnts:
                ...
                output_file.write(newTime.strftime(video_dt_format))
                output_file.write("\n")
...
with open("detected_times.txt", "r") as f:
    times = f.readlines()

times = [t.rstrip() for t in times]
times_np = np.array([np.datetime64(time) for time in times])
sns.histplot(times, bins=20, kde=True)
plt.xticks(rotation=-50)
plt.savefig("freq_of_detection.pdf")
plt.show()
